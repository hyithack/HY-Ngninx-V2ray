...
tls_config() {
	...
	while :; do
		...
		read -p "(例如：www.hyithack.com): " domain
...
shadowsocks_password_config() {
	...
	while :; do
		...
		read -p "$(echo -e "(默认密码: ${cyan}www.hyithack.com$none)"): " sspass
		[ -z "$sspass" ] && sspass="www.hyithack.com"
		case $sspass in
		*[/$]*)
			echo
...
install_v2ray() {
	...
	if [[ $local_install ]]; then
		if [[ ! -d $(pwd)/config ]]; then
			...
			echo
			echo -e " 请确保你有完整的上传 www.hyithack.com 的 V2Ray 一键安装脚本 & 管理脚本到当前 ${green}$(pwd) $none目录下"
			echo
			exit 1
		fi
...
uninstall() {
	...
	if [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f $backup && -d /etc/v2ray/233boy/v2ray ]]; then
		...
		备注...仅支持卸载使用我 (www.hyithack.com) 提供的 V2Ray 一键安装脚本
		" && exit 1
	fi
...
while :; do
	echo
	echo "........... V2Ray 一键安装脚本 & 管理脚本 by www.hyithack.com .........."
	echo
	echo "帮助说明: www.hyithack.com"
	echo
	echo "搭建教程: www.hyithack.com"
	echo
	echo " 1. 安装"
	echo
	echo " 2. 卸载"
